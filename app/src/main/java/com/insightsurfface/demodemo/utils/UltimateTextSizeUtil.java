...
//    }
    ...
    //    //这个方法会处理附加利率和利率的问题
    public static SpannableString getRateSpannableString(String rate, String subsidyRate, int textSize, boolean includeDoteNum, String errString) {
        try {
            if (TextUtils.isEmpty(subsidyRate) || 0 == Float.valueOf(subsidyRate)) {
                if (TextUtils.isEmpty(rate)) {
                    return new SpannableString(errString);
                } else {
                    return getEmphasizedString(rate + "%", "%", textSize, -1, -1);
                }
            } else {
                //不处理这个了
...
//            emphasizes[1] = "%";
                ...
                return getEmphasizedString(rate + "%+" + subsidyRate + "%", emphasizes, textSize, -1, -1);
            }
        } catch (Exception e) {
            return new SpannableString(errString);
        }
    ...
    public static SpannableString getRateSpannableString(String rate, String subsidyRate, int textSize, String errString) {
        return getRateSpannableString(rate, subsidyRate, textSize, false, errString);
    }
...
//        spannableString.setSpan(new StyleSpan(textStyle), bStart, bEnd, Spannable.SPAN_EXCLUSIVE_EXCLUSIVE);
    ...
    public static SpannableString getEmphasizedString(String text, ArrayList<String> emphasizeTexts,
        ...
        try {
            ...
            return getEmphasizedString(text, empasize, textSize, color, textStyle);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static SpannableString getEmphasizedString(String text, String emphasizeText,
                                                      int textSize, int color, int textStyle) {
        try {
    ...
    public static SpannableString getEmphasizedString(String text, String[] emphasizeTexts,
                                                      int textSize, int color, int textStyle) {
        try {
            return getEmphasizedString(new SpannableString(text), emphasizeTexts, textSize, color, textStyle);
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    ...
    public static SpannableString getEmphasizedString(SpannableString spannableString, String[] emphasizeTexts,
                                                      int textSize, int color, int textStyle) {
        try {
     ...
     * @return
     */
    public static String[] separateUnitAndNum(String text) {
        String[] res = new String[2];
        res[0] = replaceAllNum(text);
        res[1] = text.replaceAll(res[0], "");
        return res;
    }

    private static String replaceAllNum(String string) {
        String str = string.replaceAll("[0123456789.]", "");
        return str;
    }
}
